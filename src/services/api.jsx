...
const TOKEN =
  'Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiI5ODMwOTVkYjFjMzBjMTEyYWY4NmMzZTkxNjI2ODQ1ZCIsInN1YiI6IjY0YzIyOTgwMmYxYmUwMDBjYTI3NTFkYSIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ._7s7ANDTjvRolXC55iraUkBQjE0gdiu5wGUEiZzr4fE';
...
export async function fetchTrendingMovies() {
  try {
    const { data } = await axios.get(
      `${baseUrl}/trending/movie/day?${langParams}`,
      options
    );
...
export async function fetchMovieByName(name, page) {
  try {
    const { data } = await axios.get(
      `${baseUrl}/search/movie?query=${name}&${langParams}&include_adult=false&page=${page}`,
      options
    );
    return data;
  } catch (error) {
...
export async function fetchMovieCreditsById(id) {
  try {
    const { data } = await axios.get(
      `${baseUrl}/movie/${id}/credits?${langParams}`,
      options
    );
    return data;
  } catch (error) {
...
export async function fetchMovieReviewsById(id) {
  try {
    const { data } = await axios.get(
      `${baseUrl}/movie/${id}/reviews?${langParams}`,
      options
    );
    return data;
  } catch (error) {
...
export async function fetchAllMovies(genre, page) {
  try {
    const { data } = await axios.get(
      `${baseUrl}/discover/movie?with_genres=${genre}&${langParams}&include_adult=false&page=${page}`,
      options
    );
    return data;
  } catch (error) {
